...
SQS.prototype.getSQSQueueSizeInt = function(url, auth) {
	...
	return this.retrieveSQSQueueData(url, 'ApproximateNumberOfMessages', auth).then(function(data) {
...
SQS.prototype.getSQSQueueSizeNotVisibleInt = function(url, auth) {
	var sqs = new AWS.sqs(auth.props);
	return this.retrieveSQSQueueData(url, 'ApproximateNumberOfMessagesNotVisible', auth).then(function(data) {
