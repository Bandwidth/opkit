...
SQS.prototype.getSQSQueueSizeInt = function(name, auth) {
	...
	.then(function(data) {
		return Promise.resolve(data.Attributes.ApproximateNumberOfMessages);
	});
};
...
SQS.prototype.getSQSQueueSizeNotVisibleInt = function(name, auth) {
	...
	.then(function (data) {
		return Promise.resolve(data.Attributes.ApproximateNumberOfMessagesNotVisible);
	});
};
...
SQS.prototype.listQueues = function(queuePrefix, auth) {
	...
	.then(function(data) {
		return Promise.resolve(data.QueueUrls);
	});
};
