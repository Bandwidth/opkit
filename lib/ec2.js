...
/**
 * Get the tags associated with the specified EC2 instance ID.
 * @param {string} instanceId - instance ID of AWS EC2 instance
 * @param {Auth} auth - The Auth object to be used for the AWS query.
 * @returns Promise resolving to a JSON containing tag/key pair values.
 */
EC2.prototype.getTagsByID = function(instanceId, auth) {
	...
	.then(function(data) {
		...
		for (var tagIdx = 0; tagIdx < tags.length; tagIdx++) {
			tagObj[tags[tagIdx].Key] = tags[tagIdx].Value;
		}
		return Promise.resolve(tagObj);
	});
};
...
/**
 * Get info about logs of a specified EC2 instance.
 * @param {string} tagName - Name tag (Examples: Name, Environment, etc).
 * @param {string} value - Actual value of tag (Example: opkit-test-johndoe)
 * @param {Auth} auth - The Auth object to be used for the AWS query.
 * @returns Promise resolving to a String with the proper log.
 */
EC2.prototype.getLogs = function(tagName, value, auth) {
	var ec2 = new AWS.ec2(auth.props);
	var instancesArray = [];
	...
	.then(function(data) {
		...
		if (instancesArray.length === 1) {
			return ec2.getConsoleOutputPromised({InstanceId : instancesArray[0]});
		} else if (instancesArray.length > 1) {
			return Promise.reject('More than one instance specified.');
		} else {
			return Promise.reject('No instances available.');
		}
	})
	.then(function(output) {
		var buf = new Buffer(output.Output, 'base64');
		return Promise.resolve('Timestamp: ' + output.Timestamp + '\n' + buf.toString());
	});
};

/**
 * Reboots EC2 instances with the specified tag.
 * @param {string} tagName - Name tag (Examples: Name, Environment, etc).
 * @param {string} value - Actual value of tag (Example: opkit-test-johndoe)
 * @param {Auth} auth - The Auth object to be used for the AWS query.
 */
EC2.prototype.reboot = function(tagName, value, auth) {
	...
	.then(function(data) {
		return ec2.startInstancesPromised({InstanceIds: instancesArray});
	})
	.then(function(data) {
		return ec2.waitForPromised('instanceRunning', {InstanceIds: instancesArray});
	});
