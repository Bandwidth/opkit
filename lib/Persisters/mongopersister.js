...
function getMongoPersister(schema, collectionName, MONGODB_URI) {

	var mongoPersister = {};
	mongoPersister.initialized = false;

	/**
	 * Initialize a connection to the MongoDB. Also create a model based on the given collection name and schema.
	 * @returns A promise.
	 */
	mongoPersister.start = function() {
	 ...
	 */
	mongoPersister.verify = function(passedState) {
	 ...
	 */
	mongoPersister.save = function(passedState) {
		var self = this;
	 ...
	 */
	mongoPersister.retrieve = function() {
		if (this.initialized) {
	...
	return Promise.resolve(mongoPersister);
}

module.exports = getMongoPersister;