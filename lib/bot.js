...
function OpkitBot(name, cmds, persister, authFunction) {
	...
	for (var cobject in cmds){
		...
		if (current_command.names && current_command.names[0]){
			...
			this.commands[current_command.name] = current_command;
		}
	}
	// This initializes two arrays we're using to hold event handlers.
	this.handlers = [];
	this.oneoffHandlers = [];
	this.addHandler = function(pattern, logic, life){
		var newHandler = {
			pattern : pattern,
			logic : logic
		};
		...
		return Promise.resolve(newHandler);
	};
	this.addOneOffHandler = function(pattern, logic, life){
		var newHandler = {
			pattern : pattern,
			logic : logic
		};
	...
	for (var c in this.commands){
		if (!this.brain[this.commands[c].script]){
			this.brain[this.commands[c].script] = { };
		}
	}
	...
	this.messageParser = function(messagesplit, bot){
		...
		for (m=0; m<syntaxes.length; m++){
			...
			if (currentSyntax.length < messagesplit.length){
				matches = true;
				var commandPart = _.slice(messagesplit, 1, currentSyntax.length+1);
				if (_.intersection(commandPart, currentSyntax).length === currentSyntax.length){
					return Promise.resolve({
					...
					.then(function(data){
						...
						if (access){
							...
							.then(function(response){
								console.log('********');
								console.log(self.brain[command.script]);
								console.log('********');
