...
var HELPFUL_SUGGESTION_COEFFICIENT = 0.8; // Increase this to get typo suggestions less frequently.
/**
 ...
 * @param [String] cmds.names : The keywords that should be listened for.
 * @param [String] cmds.roles: An optional list of roles to control access for.
 *}
 * @param {object} persister : object that has a .save and .recover method
 * @param {object} params : Optional parameters for creating the bot
 * @param {function} params.authFunction : function which returns a list of roles to match against commands.
 * @param [String] params.logLevel : 'DEBUG' for all logs, 'PROD' for just commands, 'TEST' for no logs.
 */
...
function OpkitBot(name, cmds, persister, params) {
	params = params || {};
	var self = this;
	this.name = name;
	this.commands = { };	
	...
	if (this.logLevel === 'DEBUG'){
		this.rtm = new RtmClient(this.token, {useRtmConnect: true, dataStore : false, logLevel : 'debug'});
	} else {
		this.rtm = new RtmClient(this.token, {useRtmConnect: true, dataStore : false});
	}
 ...
 */
	...
	this.authorizationFunction = params.authFunction || function(user){
 ...
 */
	...
	this.addCron = function(cron, command, user, timeZone){
		new CronJob(cron, function(){
			self.onEventsMessage({
				text : command,
				user : user
			});
		}, null, true, timeZone);
	...
	this.messageParser = function(message, bot){
		...
		var reply;
		if (record > HELPFUL_SUGGESTION_COEFFICIENT){
			reply = "Unfortunately, that command was not found. Did you mean " + bot.name;
			for (n=0; n<currentBestSyntax.length; n++){
				reply += " " + currentBestSyntax[n];
			}
			reply += "?";
		} else {
			reply = 'That is not a command I recognize.';
		}
	...
	this.rtm.on('message', this.onEventsMessage);
