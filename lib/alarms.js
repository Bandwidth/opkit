...
/**
 * Get usage statistics for a cloudwatch metric for the past number of days.
 * Returns an Array of Objects - each object contains metric statistics about the given execution.
 * @param {Auth} auth - An auth object made using the keys and region you'd like to use.
 * @param {Object} params - AWS query params.
 * @param {Number} number_of_days - Number of days to get metrics from.
 */
Alarms.prototype.getMetricStatisticsForDay = function(auth, params, number_of_days) {
    if (number_of_days === null || number_of_days < 1) {
        number_of_days = 1;
    }
    var promises = [];
    var now = new Date();
    ...
    for (var i = 0; i < number_of_days; i++) {
        var endTime = new Date(date.getTime() + 86400000);
        params.EndTime = endTime.toISOString();
