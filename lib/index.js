 ...
 */
	...
	var functionPermissions = {}; //map of permissions to functions
	
	functionPermissions[-1] = ['createAuth', 'getAuths', 'chooseAuth', 'updateRegion',
	'updateAccessKeyId', 'updateSecretAccessKey', 'updateShortName', 'updateAuthKeys'];
	functionPermissions[1] = ['getSQSQueueSizeInt', 'getSQSQueueSizeNotVisibleInt', 'listQueues', 
	'queryAlarmsByState', 'queryAlarmsByStateReadably', 'countAlarmsByState', 'healthReportByState',
	'queryAlarmsByWatchlist', 'queryAlarmsByWatchlistReadbly', 'queryAlarmsByPrefix', 'queryAlarmsByPrefixReadably',
	'createAuth', 'getAuths', 'chooseAuth', 'updateRegion', 'updateAccessKeyId', 'updateSecretAccessKey',
	'updateShortName', 'updateAuthKeys'];
	
	this.getFunctionPermissions = function(permissionLevel) {
		return functionPermissions[permissionLevel];
	}
 ...
 */
	this.addPermission = function(user, perm) {
		...
		var i = 0;
		while (i < permissions[user].length)
		{
			if (permissions[user][i] === perm) {
				return false;
			}
			i++;
		}
 ...
 * @param {string} user - ID of user on slack.
 * @returns an array containing the user's permissions.
 */	
...
Opkit.prototype.checkPermissions = function(userPermissions, nameOfFunc) {
	var i = 0;
	var functionIsAllowed = false;
	var maxPermission = -1; //by default use user's maximum permissions
	while (i < permissions.length) {
		if (userPermissions[i] > maxPermission) 
		{
			maxPermission = userPermissions[i];
		}
		i++;
	}
	
	var j = 0;
	var allowedFunctions = this.getFunctionPermissions(maxPermission);
	while (j < allowedFunctions)
	{
		if (allowedFunctions[j] === nameOfFunc) {
...
Opkit.prototype.queryAlarmsByWatchlistReadably = function(watchlist) {
	return this.alarms.queryAlarmsByWatchlistReadbly(watchlist, this.currentAuth);
}
